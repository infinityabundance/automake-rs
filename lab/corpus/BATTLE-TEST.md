...
## NATIVE — first config.h-using GNU-free build (redir), via the full fix chain
Completed the chain to build a config.h-using project with ZERO GNU tools. **troglobit/redir now
builds end-to-end** (`autoreconf-rs -fi` + `./configure` + `make`, no GNU) — verified single-threaded.
Fixes: m4-rs nested-macro (0.1.5) → AS_IF else → AC_CONFIG_HEADER singular → config.h `@%:@` removal
→ autoheader emits resolved `PACKAGE_*` #defines (autoconf-rs 0.1.8) → driver `--force` (autom4te
cache). config.h ends correct (PACKAGE_NAME defined).

**Batch caveat (honest):** the `-P3` batch still reports 2/31 because config.h is generated by TWO
emitters and under parallelism `make` can read an intermediate (no-PACKAGE) write — a duplicate-
generation race, not a tool defect (redir's on-disk config.h is correct; single-threaded it builds).
**Eliminating the second config.h emitter is the next concrete fix** and should convert redir + the
other config.h-using configure-OK repos to GNU-free builds.

Net: configure-OK 12/31, GNU-free end-to-end builds 2 (batch) / 3 verified (redir single-threaded).
The remaining gap is the duplicate-emitter race + real `-l` probing for the lib-dependent repos.
